...
- [ ] **TOK-01**: 新建 `src/styles/_motion-tokens.scss`，声明 spring 曲线（`$ease-spring-snappy/default/gentle/stiff`）、时长分级（`$dur-micro/fast/default/enter/exit/ambient`）、CSS `linear()` spring 函数及 `cubic-bezier` 回退变量。

- [ ] **TOK-02**: 新建 `src/styles/_component-tokens.scss`，声明组件级语义变量（`$btn-pressed-scale`、`$btn-disabled-opacity`、`$cell-selected-bg`、`$modal-enter-duration` 等），所有值引用 primitive token，不硬编码数字。

- [ ] **TOK-03**: 新建 `src/styles/_mixins-lg.scss`，提供 `liquid-glass()`、`spring-btn()`、`cell-interactive()` 等 mixin，使各组件可一行引入完整五态行为。

- [ ] **TOK-04**: 同步更新 `src/utils/theme.ts` 的 `appleUi.motion` 与 `appleUi.glass`，与新 Sass token 数值保持一致（手动同提交）。
...
- [ ] **BTN-04**: disabled 态 `opacity(0.38) + pointer-events:none`，过渡时间 200ms。
...
- [ ] **CELL-04**: 用户填入数字时（非冲突），数字做轻微 **scale(0.8→1.0)** 入场，**100ms ease-out**。

### D. 弹层（Modal / Sheet）精打（Phase 4）

- [ ] **MOD-01**: 弹层卡片入场：**380ms** `cubic-bezier(0.32,0.72,0,1)`，`translateY(100%→0) + opacity(0.5→1)`，`backdrop-filter: blur(0→18px)` 同步渐进。

- [ ] **MOD-02**: 弹层卡片出场：**280ms** `cubic-bezier(0.55,0,1,0.45)`，`translateY(0→60%) + opacity→0.3`。

- [ ] **MOD-03**: 弹层蒙层（mask）入场 **300ms ease-out** `opacity(0→1) + backdrop-blur(0→8px)`，出场 **220ms ease-in** `opacity→0`。

- [ ] **MOD-04**: 弹层关闭使用 `data-state="leaving"` + **280ms setTimeout** 延迟 DOM 卸载，避免出场动画被截断闪烁。

- [ ] **MOD-05**: 弹层出现时，主内容卡片（`.content-wrap`）的 `backdrop-blur` 从 18px 降至 8px（**200ms ease**），减少 GPU 压力。
...
- [ ] **MAT-01**: `.content-wrap` 材质升级为 `backdrop-filter: blur(18px) saturate(1.8) brightness(1.02)`，保留现有 `@supports not` 实色回退。

- [ ] **MAT-02**: `.modal__card` 材质升级为 `backdrop-filter: blur(22px) saturate(2.0)`。
...
- [ ] **HERO-01**: Hero 区域使用 CSS scroll-driven animations：随页面下滚，`opacity` 渐出（滚动 0→200px 时 1→0），减少视觉干扰。

- [ ] **HERO-02**: Hero 光斑振幅降低 30%（现有 `translate(12rpx,-8rpx)` 改为 `(8rpx,-5rpx)`），更接近 iOS 26 「克制环境动效」风格。

### I. 全局 prefers-reduced-motion 完整覆盖（Phase 4）

- [ ] **A11Y-01**: 在 `src/app.scss` 或全局 `@media (prefers-reduced-motion: reduce)` 中用 `*, *::before, *::after` 全覆盖，关闭所有非必要动画。

- [ ] **A11Y-02**: 保留例外白名单：按钮 pressed opacity（80ms）、弹层淡入（200ms 仅 opacity，无位移）作为必要交互反馈。

- [ ] **A11Y-03**: 明确关闭 Hero 光斑（`animation: none !important`）、scroll-driven animations（`animation: none`）。
