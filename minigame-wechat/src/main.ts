 ...
 */
import * as engine from './sudokuEngine'
import {
  applyDailyBonus,
  loadEconomy,
  mergeSettings,
...
/** 布局仅随窗口尺寸变化，缓存避免每帧分配 */
const layoutCache: LayoutCache = { key: '', L: null }
const bgGradientCache: SizeGradientCache = { w: 0, h: 0, g: null }
const vignetteCache: SizeGradientCache = { w: 0, h: 0, g: null }
/** 避免每帧 canvas.width= 触发的 GPU 缓冲清空与状态重置 */
let lastCanvasKey = ''
/** Hero 光斑径向渐变仅随 W、heroTop 重建 */
const heroBlobCache: HeroBlobCache = { key: '', items: null }

/** 格子数字展示，避免每格 String(v) 装箱 */
const CELL_DIGITS: readonly string[] = [
  '',
  '1',
  '2',
  '3',
 ...
 */
const NUM_KEY_PALETTE: readonly NumKeyPaletteHex[] = [
...
const NUM_KEY_IDLE: NumKeyIdleStyle = {
  n0: '#ffffff',
  n1: '#f5f5f4',
...
const IDLE_RGB: { n0: Rgb; n1: Rgb } = {
  n0: hexToRgb(NUM_KEY_IDLE.n0),
  n1: hexToRgb(NUM_KEY_IDLE.n1),
}
const NUM_KEY_PALETTE_RGB: { p0: Rgb; p1: Rgb; sp: Rgb }[] = NUM_KEY_PALETTE.map(
  (p) => ({
    p0: hexToRgb(p.p0),
    p1: hexToRgb(p.p1),
    sp: hexToRgb(p.sp),
...
const NUMPAD_RGB_WHITE: Rgb = { r: 255, g: 255, b: 255 }
const NUMPAD_RGB_TXT_IDLE: Rgb = hexToRgb(NUM_KEY_IDLE.text)
const NUMPAD_RGB_STROKE_MIX: Rgb = hexToRgb('#d6d3d1')
...
function rp(W: number, n: number): number {
  return (n / 750) * W
}
...
/** 与 Taro 端共用的经济存档 */
let economy: GameEconomyState = loadEconomy()

function syncEconomy(e: GameEconomyState): void {
  economy = e
  saveEconomy(e)
}
...
let pendingConflictRevert: { row: number; col: number; revertTo: number } | null = null

function flushPendingConflict(): void {
...
function digitFromKeyEvent(res: { key?: string; code?: string }): number {
  const k = res.key
...
function onKeyboardDown(res: { key?: string; code?: string }): void {
...
function newGame(d?: Difficulty): void {
  const targetDiff: Difficulty = d !== undefined ? d : state.difficulty
  ...
  const e = applyDailyBonus(economy)
  syncEconomy(e)
...
function fillNumber(num: number): void {
...
function undo(): void {
  ...
  const ur = tryConsumeUndoProp(economy)
  if (!ur.ok) {
...
function erase(): void {
  ...
  const er = tryConsumeEraseProp(economy)
  if (!er.ok) {
...
function getLayout(): LayoutResult {
  ...
  const compact = H < 600
  const v = function (n: number): number {
  ...
  let statsY = 0
  let pills: PillRect[] = []
  let actions: ActionRect[] = []
  let metaHelp: MetaButtonLayout = { x: 0, y: 0, w: 0, h: 0 }
  let metaSettings: MetaButtonLayout = { x: 0, y: 0, w: 0, h: 0 }
  let boardSize = maxW
  let boardTop = 0
  ...
  board = {
    ...
    radius: Math.min(
      v(16),
      boardSize > 0 ? Math.max(4, boardSize * 0.035) : 0,
    ),
  }

  const numY = boardTop + boardSize + gapBoardNum

  const nums: NumKeyRect[] = []
...
function helpTextTotalHeight(lineH: number): number {
  let t = 0
...
function helpModalGeom(L: LayoutResult): HelpModalGeom {
  const W = L.W
...
function settingsModalGeom(L: LayoutResult): SettingsModalGeom {
  const W = L.W
...
function drawBackgroundGradient(W: number, H: number): void {
...
function drawHeroBlobs(W: number, topInset: number, _heroH: number): void {
...
function drawHelpModal(L: LayoutResult): void {
  const W = L.W
  const H = L.H
  const g = helpModalGeom(L)
...
function drawSettingsModal(L: LayoutResult): void {
  const W = L.W
  const H = L.H
  const g = settingsModalGeom(L)
  ctx.fillStyle = 'rgba(28, 25, 23, 0.48)'
...
function drawNumPadButton(
  ...
  let releaseBlend: number | null = null
  if (state.numReleaseAnim && state.numReleaseAnim.num === num) {
    const raw =
      (Date.now() - state.numReleaseAnim.start) / state.numReleaseAnim.duration
...
function onTouchStart(x: number, y: number): void {
  ...
  if (state.settingsOpen) {
    ...
    if (h.kind === 'set_vibration') {
      syncEconomy(mergeSettings(economy, {vibration: !economy.settings.vibration}))
      draw()
      return
    }
    return
  ...
  if (h.kind === 'cell') {
    state.actionPressed = null
    flushPendingConflict()
    const sel: SelectedCell = { row: h.row, col: h.col }
    state.selected = sel
    state.conflictCells.clear()
...
function onTouchEnd(x: number, y: number): void {
  if (state.wonOpen) {
    return
  }
...
function init(): void {
  ...
  wx.onTouchStart(function (e: WxTouchEvent) {
    const t = e.touches[0]
    if (!t) {
      return
    }
  ...
  wx.onTouchEnd(function (e: WxTouchEvent) {
  ...
  if (typeof wx.onTouchMove === 'function') {
    wx.onTouchMove(function (e: WxTouchEvent) {
      const t = e.touches[0]
      if (!t) {
        return
      }
